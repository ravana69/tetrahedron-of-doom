...
async function Draw(){
  ...
  if(!t){
    ...
    spawnSplosion = () => {
      ...
      for(m=100;m--;){
        p1 = Rn()*Math.PI*2
        p2 = Rn()<.5 ? Math.PI-Rn()**.5*Math.PI/2 : Rn()**.5*Math.PI/2
  ...
  Rl=0, Pt = .2-S(t/3), Yw = Math.PI/2 - C(t/4)*4
    ...
    obj.map(q=>{
      ...
      if(d<1.5){
        if(l=lineFaceI(tx1,ty1,tz1,tx2,ty2,tz2,q,true,false)){
          ...
          vy /= d
          vz /= d
          v[3] += vx * rv + (Rn()-.5) * variation
          v[4] += vy * rv + (Rn()-.5) * variation
          v[5] += vz * rv + (Rn()-.5) * variation
          //v[6] = 0
          spawnSparks(...l[0])
        }
      }
